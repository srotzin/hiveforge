...
const SAGA_DEFINITIONS = {
  agent_birth: {
    ...
    compensations: {
      agent_list: async (state) => {
        console.log(`[Saga] Compensating agent_list: delisting agent ${state.genome_id || 'unknown'}`);
      },
      mind_seed: async (state) => {
        console.log(`[Saga] Compensating mind_seed: archiving memory for ${state.genome_id || 'unknown'}`);
      },
      trust_register: async (state) => {
        console.log(`[Saga] Compensating trust_register: deactivating DID for ${state.genome_id || 'unknown'}`);
      },
      forge_mint: async (state) => {
        console.log(`[Saga] Compensating forge_mint: deprecating genome ${state.genome_id || 'unknown'}`);
      },
    },
  },
};
